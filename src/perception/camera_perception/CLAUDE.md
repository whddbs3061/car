...
`EgoVehicleStatus`(포트 **1911**)의 38바이트 필드. 자차가 올라가 있는 HD 맵 링크로,
**차로 단위 정답**이다. 차로를 옮기면 값이 바뀐다.

포트는 시뮬레이터 Network Settings 의 sim→user 목적지 포트를 따른다. 229바이트
`#MoraiInfo$ ... \r\n` 패킷이 들어온다. 예전에 적혀 있던 909 는 특권 포트(<1024)라
일반 사용자로는 bind 자체가 안 되고, `Receiver` 가 데몬 스레드에서 bind 하므로
스레드만 조용히 죽어 녹화가 끝날 때까지 모르고 지나간다.
